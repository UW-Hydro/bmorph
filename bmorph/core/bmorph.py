...
def blended_edcdfm(raw_x, raw_cdf, train_cdf, truth_cdf1, truth_cdf2,
                   blend_factor=0.5):
    # Type checking (note that the checking is more strict here then it
    # probably needs to be)
    assert isinstance(raw_x, pd.Series)
    assert isinstance(raw_cdf, pd.Series)
    assert isinstance(train_cdf, pd.Series)
    assert isinstance(truth_cdf1, pd.Series)
    assert isinstance(truth_cdf2, pd.Series)
    ...
    # Given u_t and train_cdf determine train_x
    train_x = pd.np.percentile(train_cdf, u_t)
    train_x[train_x < 1e-5] = 1e-5

    # Given u_t and truth_cdf determine truth_x
    truth_x = pd.np.percentile(truth_cdf1, u_t)
    truth_y = pd.np.percentile(truth_cdf2, u_t)
    truth_x[truth_x < 1e-5] = 1e-5
    truth_y[truth_y < 1e-5] = 1e-5

    # Calculate multiplier
    multiplier1 = truth_x / train_x
    multiplier2 = truth_y / train_x
    multiplier = ((blend_factor * multiplier1)
                  + ((1-blend_factor) * multiplier2))

    return pd.Series(multiplier, index=raw_x.index)


def blendmorph(raw_ts, train_ts, truth_ts1, truth_ts2,
               raw_cdf_window, training_window, raw_bmorph_window,
               blend_factor, nsmooth):
    assert isinstance(raw_ts, pd.Series)
    assert isinstance(raw_cdf_window, slice)
    assert isinstance(raw_bmorph_window, slice)
    assert isinstance(truth_ts1, pd.Series)
    assert isinstance(truth_ts2, pd.Series)
    ...
    truth_cdf1 = truth_ts1[training_window].rolling(
        window=nsmooth, min_periods=1, center=True).mean()
    truth_cdf2 = truth_ts2[training_window].rolling(
        window=nsmooth, min_periods=1, center=True).mean()
    ...
    # PDFs
    bmorph_multipliers = blended_edcdfm(
            raw_smoothed_ts[raw_bmorph_window],
            raw_smoothed_ts[raw_cdf_window],
            train_cdf, truth_cdf1, truth_cdf2, blend_factor)
